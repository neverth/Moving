...
如果只是运行Java程序的话，只需要安装jre就可以，但是JSP部署成web服务例外，因为服务器会将JSP转为servlet，需要使用JDK编译servlet
...
**hashCode**方法的所用是获取哈希码，通常用来将对象的内存地址转换为整数之后返回。其被设计来使哈希容器能更高效的工作，例如hashSet添加元素时会先判断hashCode相等，相等再equals看是否真的相等。其相等是两个对象相等的必要非充分条件。

**equals**方法用于判断两个对象是否相等，未覆盖等于"=="，覆盖比较内容相等。

在JDK的文档里写了，如果两个对象互相equals，那么这两个对象的hashCode值必须相等。默认Object的hashCode是将对象的内存地址转换为整数之后返回，每个对象的地址都会不一样，不重写就不会满足JDK文档里的要求。
...
IoC 就是控制反转，简单来说就是将原来代码里需要手动创建管理的对象、托付给容器来帮忙实现，这就需要创建一个容器并且需要一种描述让容器知道要创建的对象间的关系。因此IoC容器需要具备两个基本能力，①通过描述管理Bean，包括发布和获取Bean。②通过描述完成Bean之前的依赖关系。

IoC 的实现方式有依赖注入和依赖查找，由于依赖查找使用的很少，因此 IoC 也叫做依赖注入。依赖注入指对象被动地接受依赖类而不用自己主动去找，对象不是从容器中查找它依赖的类，而是在容器实例化对象时主动将它依赖的类注入给它。

在Spring中，要求所有的IoC容器都需要实现BeanFactory接口。接口中定义了通过type或者是name获取bean，
...
#### 过滤器和拦截器区别？
两者比较相似，其最大的区别在于过滤器是在Servlet规范中定义的，是由Servlet容器支持的，拦截器是在Spring容器内的，由Spring框架支持。
...
请求路径 tomcat->filter->servlet->interceptor-controller
...
连接器 - 连接处理、授权认证、安全等
查询缓存 - 命中缓存直接返回结果
分析器 - 语法分析和词法分析（将sql中对应字符串锁表达的含义分析出来）
优化器 - 发现执行sql查询的最佳方案（查询计划），例如选择索引
执行器 - 操作引擎，返回结果

-------存储引擎层------
存储引擎 - 存储数据，提供读写的接口，如果开启查询缓存则会缓存查询结果
...
**缓存穿透**是指查询一个根本不存在的数据，不管是在缓存层或者是存储层都不存在。客观原因通常处于容错的考虑，如果从DB中查到空数据并不会把空结果写入缓存中。主观原因恶意攻击爬虫等造成大量空命中。自身业务代码或者是数据出现问题。

解决方案：

1. 缓存空对象解决其客观原因，DB中查到空数据之后也直接写入到缓存中。① 但是消耗了更多的内存 - 设置一个较短的过期时间。②缓存和DB的的数据会有不一致现象 - 利用消息队列
2. 布隆过滤器，是一个概率型数据结构，特点是高效地插入和查询，传统一般不支持删除，可以用来告诉你 “某样东西一定不存在或者可能存在”。其数据结构就是一个很大的Bit数组和几个不一样的哈希函数，一个元素通过不一样的哈希函数可以得到几个不一样的下标。插入时下标都置为一，查询时只要存在下标不为一代表一定不存在。在缓存层之前加一层布隆过滤器，如果不存在直接返回

**缓存雪崩**指数据大范围过期失效或者缓存直接不能提供服务，于是所有请求都会到达DB，造成DB级联宕机。

解决方案：

1. 交叉失效时间
2. 保证缓冲层高可用，sentinel和cluster都可以实现高可用
3. 重要资源限流并降级
4. 提前演练

**缓存击穿**是缓存雪崩的一个特例，某个单独热点过期失效，会有大量请求到达DB，造成DB故障。

解决方案，二级缓存：对于那些热点key设置二级缓存，并错开和以及缓存的失效时间，是请求不会同时穿过两层缓存到达DB

## 操作系统

### 进程间通信方式有哪些？

**① 管道**，linux下命令`ps auxf | grep mysql` 其实就是一个**匿名管道**，也可以通过`mkfifo name`创建**命名管道**。其实现是通过pipe系统调用函数创建，其返回两个描述符，fd[0]用于读，fd[1]用于写。pipe函数返回的两个描述符在同一进程中，并不能做到进程通信，所以需要fork子进程，子进程会复制父线程的文件描述符。用`mkfifo`命令可以创建一个管道文件，因此命名管道，它可以在不相关的进程间也能相互通信。

缺点：效率低、不适合进程间频繁地交换数据、会阻塞、匿名管道的通信范围是存在父子关系的进程。优点：简单。
...
一般CPU100代表可能存在死循环，使用阿里的arthas可以很好的解决，首先dashboard命令查看占用CPU最高的线程，接下里使用thread命令查看对应CPU最高线程的堆栈信息，同过堆栈信息就可以找到对应可能死循环的方法。当然arthas还有很多其他命令，monitor方法执行监控，watch数据监控，sc看到已经加载的类信息等。


top命令也可以，首先top查看高cpu的进程pid，再top -h -p pid查看对应占用cpu高的线程，并把线程pid转成16进制，最后通过jstack 查看线程的堆栈信息。
...
1. B+树索引，
2. hash索引，基于哈希表实现，只有全值匹配才有效。以链表的形式解决冲突。查找速度非常快 O(1)
3. 全文索引，查找的是文本中的关键词，而不是直接比较索引中的值，类似于搜索引擎做的事情。
4. 空间数据索引（R-Tree），MyISAM支持空间索引，可以用作地理数据存储，会从所有维度来索引数据，有效的使用任意维度来组合查询。

**从物理存储角度：**

1. 聚簇索引，InnoDB同一个结构保存了索引和数据行，叶子节点的data域存放了行的全部数据。
...
2. 非聚簇索引，MyISAM索引文件和数据文件是分离的，叶子节点的data域存放的是行数据的地址
...
1. 普通 /单列索引，普通索引的唯一作用就是为了快速查询数据，一张表允许创建多个普通索引，并允许数据重复和NULL。`INDEX index_name ( column )`。
2. 唯一索引，唯一索引也是一种约束。唯一索引的属性列不能出现重复的数据，但是允许数据为NULL，一张表允许创建多个唯一索引。`UNIQUE ( column )` 
...
1. 主键索引，数据表的主键列使用的就是主键索引。InnoDB中，当没有显式的指定表的主键时，InnoDB会自动先检查表中是否有唯一索引的字段，如果有，则选择该字段为默认的主键，否则InnoDB将会自动创建一个6Byte的自增主键。
2. 二级 / 辅助索引，除开主键索引之外的都叫二级索引。在MyISAM中二级索引和主键索引的结构大致相同。在InnoDB中二级索引的叶子节点存储的是主键值，通过主键值定位行数据，需要两次索引查找。使用主键值当做指针会让二级索引占更多的空间，但是移动行时无需更新二级索引。
3. 覆盖索引，如果一个索引包含（或者说覆盖）所有需要查询的字段的值，我们就称之为“覆盖索引”。覆盖索引即需要查询的字段正好是索引的字段，那么直接根据该索引，就可以查到数据了， 而无需回表查询。

### SQL怎么调优？

1. 打开慢查询日志，查看慢查询
2. 首先在线下环境**explain看一下执行计划**，是否符合预期，例如查看①key字段是否使用到索引，使用到什么索引。②type字段是否为ALL全表扫描。③row字段扫描的行数是否过大，估计值。MySQL数据单位都是页，使用采样统计方法。④extra字段是否需要额外排序，就是不能通过索引顺序达到排序效果；是否需要使用临时表等。⑤如果是组合索引的话通过key_len字段判断是否被完全使用。
3. **了解业务场景**。看业务怎么使用这个sql，做针对性优化。
4. **排除缓存的干扰**。如果线程RT（响应时间）快时慢的话就可能是缓存的问题，因为对表进行更新的话先关缓存都会失效
5. **使用覆盖索引**。覆盖索引指的是一个索引包含所需要查询的字段的值，根据该索引可以直接查到数据，并不需要回表查询。比如二级索引都会包含主键值，通过二级索引查找主键值就会用到覆盖索引，一次查表，但是假如是查找 所有(*) 的话就还是需要回表。
6. **使用联合索引**。比如根据名称查库存就可以建立联合索引，不需要根据名称查出主键之后在查库存了，但需要考虑业务场景，避免占据较大空间。
7. **注意最左前缀原则**，按照定义的顺序写sql。如果一个模糊查询只使用到一个联合索引的最左索引，那这样还是能用到这个联合索引，并不需要新建一个单独的索引。
8. **选择合适的索引列顺序**。当不需要考虑排序和分组时，将选择性最高的列放到索引的最前列通常是很好的。
9. **索引下推**，mysql5.6之后官方自动优化。比如联合索引(name,age)，根据name的like查询并且age在一定区间内查找所有匹配的行数据时，就只有name可以用到索引，age并不会。在优化后，name用到索引之后，会直接再从索引中匹配满足条件的age，之后再回表查询，这样需要回表查询的数据就相比减少了。其实就是充分利用了索引中的数据，尽量在查询出整行数据之前过滤掉无效的数据。
10. **使用前缀索引**。当要给字符串加索引时，可以使用前缀索引，节省资源占用。如果前缀区分度不高可以倒序存储或者是存储hash。
11. **使用独立的列**。索引列不能是表达式的一部分，也不能是函数的参数，始终将索引列单独放在比较符号的一侧。
12. **注意隐式类型转换**。比如id是字符类型，查询的使用使用int类型会相当于加了类型转换函数，用不上索引。两个表的字符集不一样也会导致，例如utf8mb4（可以超过3字节）和utf8（最多3字节）
13. 被频繁更新的字段应该慎重建立索引，不被经常查询的字段没有必要建立索引。
14. 遵循索引设计准则**三星索引**，但一般难以满足，需要依赖实际成本和业务场景。① WHERE 后面参与查询的列可以组成了单列索引或联合索引。② 避免排序，即如果 SQL 语句中出现 order by colulmn，那么取出的结果集就已经是按照 column 排序好的，不需要再生成临时表。③尽量使用覆盖索引 
...
1. 资源静态化，一般秒杀大部分都是静态资源，可以把能放进CDN尽量都放进CDN。CDN就是内容分发器，DNS解析的过程中会选择离用户时延最低的CDN
2. 秒杀链接动态化，但是这个问题治标不治本，只能过滤一些没有耐心的羊毛党
3. 前端限流，没到时间按钮至灰，点了几下停一下才能继续点

-----------后端--------------

1. 负载均衡，使用Nginx高性能web服务器分发请求并拦截部分恶意请求（IP）
2. 后端限流，加入阿里的sentinel组件，假如1000库存，10万人抢，可以一次放一万人进来。
3. 风控，依赖风控团队，从一万人中选最不像羊毛党的1000人放行

---------后端处理-----------

1. 使用Redis集群，主从同步，读写分离，哨兵监控，持久化
2. 库存预热，秒杀的本质就是对库存的抢夺，将库存加载到redis中，让整个流程都redis中处理，可以使用事务，秒杀结束了在异步修改DB
3. 消息队列，削峰填谷，修改库存的请求放入消息队列，然后在一点点消费修改库存
4. 数据库，为秒杀服务单独建立一个数据库，表的设计尽可能简单，并优化索引。
5. 分布式事务保证所有结果一致，因为上面的任何一个步骤都有可能出错，所以需要分布式事务保证数据一致。
6. 限流(计数器，滑动窗口，漏桶算法，令牌桶)，降级(接口长时间无反应，返回默认方法)，熔断(拒绝后续服务)，隔离(降级，防止影响其他接口)。
7. 服务单一，保证高可用
...
就像我们平时用框架开发项目一样，也需要MVC和三层架构。复杂的系统需要分层，因为每一层都需要专注于一类事情。我们的网络分层的原因也是一样，每一层只专注于做一类事情。

1. **各层之间相互独立**：各层之间相互独立，各层之间不需要关心其他层是如何实现的，只需要知道自己如何调用下层提供好的功能就可以了。
2. **提高了整体灵活性** ：每一层都可以使用最适合的技术来实现，你只需要保证你提供的功能以及暴露的接口的规则没有改变就行了，高内聚，低耦合。
3. **大问题化小** ：分层可以将复杂的网络间题分解为许多比较小的、界线比较清晰简单的小问题来处理和解决。这样使得复杂的计算机网络系统变得易于设计，实现和标准化。这个和我们平时开发的时候，一般会将系统功能分解，然后将复杂的问题分解为容易理解的更小的问题是相对应的，这些较小的问题具有更好的边界（目标和接口）定义。

之前在本书上看到过，计算机科学领域的任何问题都可以通过增加一个间接的中间层来解决，计算机整个体系从上到下都是按照严格的层次结构设计的。

### MVC和三层架构区别？

三层架构是指：

* View视图层：用于接收用户提交请求的代码在这里编写。
* Service服务层：系统的业务逻辑主要在这里编写。
* Dao持久层：直接操作数据库的代码在这里编写。
...
**TCC**，2PC 和 3PC 都是数据库层面的，而 TCC 是业务层面的分布式事务。TCC指的是`Try - Confirm - Cancel`
...
一个线程可以监视多个描述符，一旦某个描述符就绪（一般是读或写就绪），能够通知程序进行相应的读写操作，但本质上都是同步I/O。没有描述符就绪时会阻塞线程。多路是指网络连接，复用是指复用同一个线程。

**select**，调用select函数后根据传参超时时间可能阻塞，直到有描述符就绪，或者到达超时时间之后返回，当函数返回之后通过遍历fdset来找到就绪的描述符。优点：具有良好的跨平台性。缺点：

* 能够监视的文件描述符的数量存在最大限制，一般1024
* 对socket扫描是线性扫描（轮询），效率较低 O(n)
* 需要维护一个用来存放大量fd的数据结构，这样会使得用户空间和内核空间在传递该结构时复制开销大，每次select都会拷贝

**poll**，本质上和select区别不大，它将用户传入的数组拷贝到内核空间，然后遍历fd对应的设备状态，如果设备就绪则在设备等待队列中加入一项并继续遍历，如果遍历完所有fd后没有发现就绪设备，则挂起当前进程，直到设备就绪或者主动超时。优点，基于链表存储，没有监视数量的显著。缺点：

* 和select一样，复制开销大，每次poll都会拷贝
* 线性扫描 O(n)

**epoll**，调用epoll_create时，内核会创建一个红黑树用于存储epoll_ctl传来的fd，并且建立一个链表用于存储准备就绪的fd（通过内核回调函数添加就绪的fd），当epoll_wait调用时，就绪链表存在数据则返回，无数据就阻塞，或者等到超时时间后返回。优点：

* 没有最大并发连接的限制，1G内存可以10万个端口
* 效率提升，基于回调而不是轮询，不会随fd增多效率下降O(1)
* 减少复制开销，利用mmap文件映射内存加速与内核空间的消息传递，fd首次调用epoll_ctl的时候会拷贝，之后调用epoll_wait并不会拷贝
